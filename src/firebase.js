...
export const userProfileHandler = async (userAuth, additionalInfo) =>{ //take userAuth Object and add to firestore
  const cartOrderTotalPriceArr = additionalInfo.reduce((acom,current) => acom + current.totalPriseOrder,0);
  const cartOrderTotalCountArr = additionalInfo.reduce((acom,current) => acom + current.totalCountOrder,0);
  if(!userAuth) return;
  ...
  if(!snapshot.exists){
    ...
    await currentUserRef.set({   //create new
        email,
        date : currentTime,
